...
> {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    // Init the state
    const newState = {} as FontIconPickerState;
   ...
   */
  handleOuterClick = /* istanbul ignore next */ /* istanbul ignore next */ event => {
    const { target } = event;
    // is it inner?
  ...
  handleEscapeKeyboard = /* istanbul ignore next */ /* istanbul ignore next */ event => {
    if (event.keyCode === 27) {
      this.closeDropdown();
    }
  };
  isClickWithin = /* istanbul ignore next */ /* istanbul ignore next */ target => {
    if (
  ...
  handleDropDown = (isOpen, set = true) => {
    // Init the state
    const newState = { isOpen } as FontIconPickerState;
    // Listen for theme change
    newState.elemClass = FontIconPicker.getDerivedClassName(
      "rfip",
      this.props.theme,
  ...
  handlePortalEnter = /* istanbul ignore next */ /* istanbul ignore next */ node => {
  ...
  handlePortalEntering = /* istanbul ignore next */ /* istanbul ignore next */ node => {
    const selectorNode = node.childNodes[0];
  ...
  handlePortalEntered = /* istanbul ignore next */ /* istanbul ignore next */ node => {
    // reset style
    const selectorNode = node.childNodes[0];
    this.resetPortalStyle(selectorNode);
    // focus on search
  ...
  handlePortalExit = /* istanbul ignore next */ /* istanbul ignore next */ node => {
    const selectorNode = node.childNodes[0];
    this.resetPortalStyle(selectorNode);
    const { height } = getComputedStyle(selectorNode);
    selectorNode.style.maxHeight = height;
  };
  handlePortalExiting = /* istanbul ignore next */ /* istanbul ignore next */ node => {
    const selectorNode = node.childNodes[0];
